...
import { CuentaBancaria, MovimientoBancario, TipoCuenta, TipoMovimientoBancario } from '../domain/types';
...
const MOCK_CUENTAS: CuentaBancaria[] = [
    ...
    },
    {
        ...
        saldoDisponible: 7500.00, // Diferencia por cheques girados no cobrados
        moneda: 'USD',
        createdAt: '', updatedAt: '', createdBy: ''
    }
];
...
export class InMemoryBancosRepository {
    async getCuentas(empresaId: string): Promise<CuentaBancaria[]> {
        await new Promise(resolve => setTimeout(resolve, 300));
        return MOCK_CUENTAS.filter(c => c.empresaId === empresaId);
    }

    async getMovimientos(cuentaId: string): Promise<MovimientoBancario[]> {
        await new Promise(resolve => setTimeout(resolve, 200));
        return MOCK_MOVIMIENTOS.filter(m => m.cuentaId === cuentaId);
    }
    ...
    async saveMovimiento(movimiento: MovimientoBancario): Promise<void> {
        ...
        if (cuenta) {
            if (movimiento.esEgreso) {
                ...
                // Si es cheque, afecta disponible hasta que se cobre? 
                // Simplificación: afecta ambos
