...
export class InMemoryConfiguracionRepository {
    ...
    async getPuntosEmision(empresaId: string): Promise<PuntoEmision[]> {
        await new Promise(resolve => setTimeout(resolve, 200));
        // En un backend real, haríamos join. Aquí filtramos manualmente.
    ...
    async getFechaCierre(empresaId: string): Promise<string> {
