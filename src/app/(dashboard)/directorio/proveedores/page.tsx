...
export default function DirectorioProveedoresPage() {
    ...
    useEffect(() => {
        if (currentEmpresa) {
            cargarTerceros(TipoTercero.PROVEEDOR);
        }
    }, [currentEmpresa?.id]);
    ...
    const handleExport = () => {
        ...
        link.setAttribute("download", `proveedores_${currentEmpresa?.razonSocial.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Directorio de proveedores.</p>
                </div>
            </div>
            ...
            {modalOpen && (
                <TerceroModal
                    ...
                    onSave={() => { setModalOpen(false); cargarTerceros(TipoTercero.PROVEEDOR); }}
                    empresaId={currentEmpresa.id}
                    prefixedType={TipoTercero.PROVEEDOR}
                />
            )}
            <ConfirmModal
                ...
                message="¿Está seguro de desactivar este proveedor?"
