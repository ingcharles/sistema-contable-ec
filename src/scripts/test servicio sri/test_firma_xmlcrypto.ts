/**
 * Script de test para firma usando xml-crypto directamente
 */

import { db } from '../../shared/infrastructure/database/postgresql';
import { SignatureServiceXmlCrypto } from '../../modules/facturacion/domain/services/test api sri/SignatureServiceXmlCrypto';
import fs from 'fs';
...
async function consultarAutorizacion(claveAcceso: string): Promise<string> {
    const soap = `<?xml version="1.0" encoding="UTF-8"?>
...
async function main() {
    console.log('=== TEST FIRMA CON XML-CRYPTO ===\n');
...
<comprobanteRetencion id="comprobante" version="2.0.0">
  ...
  <docsSustento>
    ...
    try {
        ...
        console.log('\n2. Firmando con xml-crypto...');
        const xmlFirmado = await SignatureServiceXmlCrypto.signXml(xml, {
            p12Base64,
            passwordP12: password
        });
        
        fs.writeFileSync('test_xmlcrypto_firmado.xml', xmlFirmado);
        console.log('✅ XML firmado guardado en test_xmlcrypto_firmado.xml');
        ...
        fs.writeFileSync('test_xmlcrypto_resp_recepcion.xml', respRecepcion);
        ...
        // Extraer mensajes
        const mensajes = [...respRecepcion.matchAll(/<mensaje>([\s\S]*?)<\/mensaje>/g)];
        ...
        if (estado === 'RECIBIDA') {
            ...
            const respAuth = await consultarAutorizacion(claveAcceso);
            console.log('\n=== RESPUESTA AUTORIZACIÓN ===');
            console.log(respAuth);
            
            fs.writeFileSync('test_xmlcrypto_resp_auth.xml', respAuth);
        }
