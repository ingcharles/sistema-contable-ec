...
/**
 * Servicio de firma XAdES-BES v1.3.2 para SRI Ecuador
 * Usa xml-crypto para canonicalización C14N correcta
 */
export class SignatureServiceSRI {
  
  /**
   * Firma un documento XML siguiendo XAdES-BES v1.3.2 para SRI Ecuador
   * Requisitos SRI:
   * - Canonicalización: C14N (http://www.w3.org/TR/2001/REC-xml-c14n-20010315)
   * - Algoritmo de firma: RSA-SHA1
   * - Digest: SHA1
   * - Estructura: ds:Signature con etsi:QualifyingProperties
   */
  static async signXml(xml: string, config: { p12Base64: string, passwordP12: string }): Promise<string> {
    try {
      ...
      const signedInfoId = `SignedInfo-${this.randomId()}`;
      const signedPropertiesId = `SignedProperties-${this.randomId()}`;
      const objectId = `Object-${this.randomId()}`;
      const referenceId = `Reference-${this.randomId()}`;
      const keyInfoId = `KeyInfo-${this.randomId()}`;
      ...
      // 2. Parsear XML y calcular digest del documento usando C14N
      const doc = new DOMParser().parseFromString(xml, 'text/xml');
      const rootElement = doc.documentElement;
      
      // Usar xml-crypto C14N para canonicalizar
      ...
      // SignedProperties para calcular digest (necesita namespaces para C14N independiente)
